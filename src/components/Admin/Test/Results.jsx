...
function TestResults() {
  ...
  const iconhandleResult = (row) => {
    console.log("Result clicked for row:", row);
    setSelectedRow(row);
  ...
  const columns = [
    ...
    columnHelper.accessor("manage", {
      ...
      Cell: ({ cell }) => (
        <div>
          <IconButton onClick={() => iconhandleResult(cell.row.original)}>
            <EventNoteIcon />
          </IconButton>
        </div>
      ),
  ...
  return (
    ...
    <Container
      maxWidth="xl"
      style={{ backgroundColor: "#F0F4FB", marginLeft: "7%", marginTop: "1%" }}
    >
      <Grid item xs={12}>
        <Typography
          variant="h6"
